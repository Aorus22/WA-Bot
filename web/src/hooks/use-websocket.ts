...
const getWsUrl = () => {
	...
	if (typeof window !== "undefined") {
		...
		if (port) return `${protocol}//localhost:${port}/ws`

		// Fallback for Electron file:// (origin is "null")
		const origin = window.location.origin
		if (origin && origin !== "null" && origin !== "file://") {
			return `${protocol}//${window.location.host}/ws`
		}
		// Desktop fallback before port discovery
...
export function useWebSocket(userId?: string, onMessage?: (msg: WSMessage) => void): WebSocketHook {
	...
	const connect = useCallback(() => {
		...
		// Re-evaluate WS URL each time (port changes in Electron desktop mode)
		const url = getWsUrl()
		if (!url) {
